...
// Custom Floating Neobrutalist Zoom & Action Panel with Select/Pan Modes
function CanvasControls({ isSelectMode, setIsSelectMode }) {
  ...
  const handleInsertBlock = (nodeType) => {
    ...
    const newBlockNode = {
      ...
      data: nodeType === 'transformNode' 
        ? { type: 'UPPER', script: '' } 
        : nodeType === 'filterNode' 
        ? { condition: "{col} = '1st Year'" } 
        : nodeType === 'conditionNode'
        ? { newColumnName: 'amount', rules: [{ operator: '=', value: 'paid', thenVal: '1000' }], elseVal: '0' }
        : {}
    };
        ...
        <div className={styles.drawerBody}>
          <section className={styles.drawerSection}>
            <div className={styles.sectionHeader}>
              ...
              <h4>Tinkercad Click-to-Route Wires 🔌</h4>
            </div>
          ...
          <section className={styles.drawerSection}>
            <div className={styles.sectionHeader}>
              ...
              <h4>Row Mismatching (Join Block)</h4>
            </div>
            <p>If rows are sorted differently or mismatch across sheets, use a <strong>Join Block</strong>:</p>
            <p>1. Spawn a Join Block using the cyan <strong>"+ Add Join Block"</strong> button or directly inside a connection line.</p>
            <p>2. Connect the column from your base sheet (e.g. <code>names.name</code>) to the <strong>base</strong> (yellow) handle of the block.</p>
            <p>3. Connect the column from your directory sheet (e.g. <code>mobiles.name</code>) to the <strong>match</strong> (purple) handle of the block.</p>
            <p>4. Connect the right handle of the Join Block to your Output column. Rows will align case-insensitively, trimming spaces automatically!</p>
          </section>
        </div>
      </div>
