...
import styles from './FilterNode.module.css';

export default function FilterNode({ id }) {
  const { 
    ...
    <div className={styles.node}>
      ...
      <div className={styles.capsule}>
        <div className={styles.titleWrapper}>
          <Filter size={11} className={styles.titleIcon} />
          <span className={styles.title}>FLTR</span>
        </div>
        <div className={styles.actions}>
          <button 
            className={styles.iconBtn} 
            onClick={() => setInspectorNodeId(id)} 
            title="Inspect & Edit Filter Condition"
          >
            <Eye size={12} />
          </button>
          <button 
